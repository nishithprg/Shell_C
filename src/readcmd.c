...
static void freecmd(struct cmdline *s)
{
	...
	/* We are not freeing bg since bg's and seq's common elements point to the same commands. */
}
...
struct cmdline *readcmd(void)
{
	...
	while ((w = words[i++]) != 0) {
		...
		case '&':
			...
			/* Add a background job to the list of background jobs. */
			bg = xrealloc(bg, (bg_len + 2) * sizeof(char **));
			bg[bg_len++] = cmd;
			bg[bg_len] = 0;

			/* Add the command to the sequence of commands. */
			seq = xrealloc(seq, (seq_len + 2) * sizeof(char **));
			seq[seq_len++] = cmd;
			seq[seq_len] = 0;
	...
	if (cmd_len != 0) {
		...
		seq[seq_len++] = cmd;
		seq[seq_len] = 0;
	} else if (seq_len != 0 && strcmp(words[i-2], "&")) {
		s->err = "misplaced pipe";
		i--;
		goto error;
	} else
...
int is_bg_job(char **cmd, char ***bg){
	/* Go through the list of background jobs. */
	for(int i = 0; bg[i] != 0; i++){
		int res = 0;
		int j;
		for(j = 0; cmd[j] != 0 && bg[i][j] != 0; j++){
			res += strcmp(cmd[j], bg[i][j]);
		}
		/* Command is a background job. */
		if(res == 0 && bg[i][j] == 0) return 1;
	}
	/* Command is not a background job. */
	return 0;
}